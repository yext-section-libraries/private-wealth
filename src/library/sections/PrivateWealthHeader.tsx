...
const PrivateWealthHeaderFields: YextFields<PrivateWealthHeaderProps> = {
  variant: {
...
const PrivateWealthHeaderComponent: PuckComponent<PrivateWealthHeaderProps> = (
  props,
) => {
  ...
  const getOutlineCtaStyle = (cta: {
    styles?: { variant?: string | null };
  }): React.CSSProperties | undefined =>
    cta.styles?.variant === "secondary"
      ? {
          color: isDarkColor(props.section.backgroundColor, streamDocument)
            ? "#fff"
            : "#000",
        }
      : undefined;
        ...
        <div className="flex items-center gap-2">
          {utilityLinks.map((item) => (
            <Link
              ...
              className="inline-flex h-8 shrink-0 items-center justify-center transition-opacity hover:opacity-80"
              style={{
...
export const PrivateWealthHeader: YextComponentConfig<PrivateWealthHeaderProps> =
  {
    label: msg("components.header", "Header"),
    fields: PrivateWealthHeaderFields,
    defaultProps: {
    ...
    render: (props) => (
      <AnalyticsScopeProvider
        name={`PrivateWealthHeader${getAnalyticsScopeHash(props.id)}`}
      >
        <PrivateWealthHeaderComponent {...props} />
      </AnalyticsScopeProvider>
    ),
  };

export const config: SectionConfig = {
  id: "PrivateWealthHeader",
  displayName: "Header",
  description: "Header",
